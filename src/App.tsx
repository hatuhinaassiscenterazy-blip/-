...
const FloatingPolaroids = () => {
  ...
  return (
    <div className="fixed inset-0 pointer-events-none z-0 overflow-hidden">
      {images.map((src, i) => {
        // Distribute strictly to left and right sides
        const isLeft = i % 2 === 0;
        const sideIndex = Math.floor(i / 2); // 0, 1, 2 for each side
        
        // Stagger horizontal positions slightly so they aren't in a perfect straight line
        const positionX = isLeft ? 2 + sideIndex * 4 : 88 - sideIndex * 4; 
        
        // Fixed duration for all so they move at the exact same speed and never catch up
        const fixedDuration = 30;
        
        // Space them out evenly (10 seconds apart) so they never overlap vertically
        // Right side is offset by 5 seconds so it doesn't look perfectly mirrored
...
const BeforeAfterSlider = ({ beforeImage, afterImage, onExpand }: { beforeImage: string, afterImage: string, onExpand?: (src: string) => void }) => {
  ...
  return (
    ...
    >
      <img src={afterImage} alt="Восстановленное фото" className="absolute inset-0 w-full h-full object-cover select-none pointer-events-none" />
      <div 
        className="absolute inset-0 w-full h-full overflow-hidden" 
        style={{ clipPath: `inset(0 ${100 - sliderPosition}% 0 0)` }}
      >
        <img src={beforeImage} alt="Оригинальное архивное фото" className="absolute inset-0 w-full h-full object-cover select-none pointer-events-none" />
      </div>
...
const ImageCarousel = ({ images, altPrefix, aspectRatio = "aspect-[4/5]", onExpand }: { images: string[], altPrefix: string, aspectRatio?: string, onExpand?: (src: string) => void }) => {
  ...
  return (
    <div className={`relative ${aspectRatio} rounded-3xl overflow-hidden mb-6 border-glass group/carousel bg-[#1c1715]`}>
      ...
      <button 
        onClick={prevSlide}
        className="absolute left-3 top-1/2 -translate-y-1/2 w-8 h-8 rounded-full bg-[rgba(28,23,21,0.5)] backdrop-blur-sm border border-[rgba(255,249,240,0.3)] flex items-center justify-center text-[#fff9f0] opacity-0 group-hover/carousel:opacity-100 transition-opacity hover:bg-[rgba(255,249,240,0.2)] z-10"
      >
        <ChevronLeft className="w-5 h-5" />
      </button>
      <button 
        onClick={nextSlide}
        className="absolute right-3 top-1/2 -translate-y-1/2 w-8 h-8 rounded-full bg-[rgba(28,23,21,0.5)] backdrop-blur-sm border border-[rgba(255,249,240,0.3)] flex items-center justify-center text-[#fff9f0] opacity-0 group-hover/carousel:opacity-100 transition-opacity hover:bg-[rgba(255,249,240,0.2)] z-10"
      >
        <ChevronRight className="w-5 h-5" />
      </button>

      {/* Dots */}
      <div className="absolute bottom-4 left-0 right-0 flex justify-center gap-2 opacity-0 group-hover/carousel:opacity-100 transition-opacity z-10">
        {images.map((_, idx) => (
          <button 
            key={idx}
    ...
    <div className="relative aspect-video rounded-3xl overflow-hidden border-glass shadow-2xl bg-[#1c1715]">
      {!isPlaying && (
        <div className="absolute inset-0 cursor-pointer group" onClick={handlePlay}>
          ...
          <div className="absolute inset-0 bg-black/40 flex items-center justify-center transition-background group-hover:bg-black/20">
...
const Calculator = () => {
  ...
  const calculateTotal = () => {
    ...
    if (selectedServices.includes('selfie')) {
  ...
  const getOrderText = () => {
    ...
    const selectedLabels = selectedServices.map(id => {
      const opt = baseOptions.find(o => o.id === id);
      ...
      {selectedServices.includes('selfie') && (
        <div className="mb-8 p-6 rounded-xl border border-[rgba(255,249,240,0.1)] bg-[rgba(0,0,0,0.2)]">
          <div className="flex items-center justify-between">
            <span className="text-[#fff9f0]">Дополнительные участники (+150 ₽/чел)</span>
            <div className="flex items-center gap-4 bg-[#1c1715] rounded-full px-2 py-1 border border-[rgba(255,249,240,0.1)]">
              <button onClick={(e) => { e.preventDefault(); setAdditionalPeople(Math.max(0, additionalPeople - 1)); }} className="w-8 h-8 rounded-full flex items-center justify-center hover:bg-[rgba(255,255,255,0.05)] text-[#fff9f0] transition-colors">-</button>
              <span className="w-4 text-center font-medium text-[#fff9f0]">{additionalPeople}</span>
              <button onClick={(e) => { e.preventDefault(); setAdditionalPeople(additionalPeople + 1); }} className="w-8 h-8 rounded-full flex items-center justify-center hover:bg-[rgba(255,255,255,0.05)] text-[#fff9f0] transition-colors">+</button>
            </div>
          </div>
        </div>
      )}

      <div className="pt-8 border-t border-[rgba(255,249,240,0.1)]">
        <div className="flex items-end justify-between mb-8">
          <span className="text-xl text-[#f3e8d8]">Итоговая сумма:</span>
          <span className="text-4xl font-serif text-[#fff9f0]">{calculateTotal()} ₽</span>
        </div>
        
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4 relative z-20">
          <a href={`https://vk.ru/im?sel=-240849361&text=${getOrderText()}`} target="_blank" rel="noreferrer" className="flex items-center justify-center gap-2 py-4 rounded-xl bg-[rgba(255,255,255,0.03)] border border-[rgba(255,249,240,0.2)] hover:bg-[#fff9f0] hover:text-[#120f0e] text-[#fff9f0] transition-all font-medium shadow-[0_0_15px_rgba(255,249,240,0.05)] hover:shadow-[0_0_20px_rgba(255,249,240,0.2)]">
            Заказать в ВК
          </a>
          <a href={`https://t.me/hatyhinavika?text=${getOrderText()}`} target="_blank" rel="noreferrer" className="flex items-center justify-center gap-2 py-4 rounded-xl bg-[rgba(255,255,255,0.03)] border border-[rgba(255,249,240,0.2)] hover:bg-[#fff9f0] hover:text-[#120f0e] text-[#fff9f0] transition-all font-medium shadow-[0_0_15px_rgba(255,249,240,0.05)] hover:shadow-[0_0_20px_rgba(255,249,240,0.2)]">
            Заказать в Telegram
          </a>
          <a href={`https://clck.ru/3Vh6Je`} target="_blank" rel="noreferrer" className="flex items-center justify-center gap-2 py-4 rounded-xl bg-[rgba(255,255,255,0.03)] border border-[rgba(255,249,240,0.2)] hover:bg-[#fff9f0] hover:text-[#120f0e] text-[#fff9f0] transition-all font-medium shadow-[0_0_15px_rgba(255,249,240,0.05)] hover:shadow-[0_0_20px_rgba(255,249,240,0.2)]">
            Заказать в Max
          </a>
        </div>
      </div>
          ...
          <div className="space-y-8">
            <article className="bg-[#1c1715] border-glass rounded-3xl p-6 md:p-12 flex flex-col md:flex-row gap-12 items-center">
              ...
              <div className="w-full md:w-1/2 md:max-w-[400px]">
                <ImageCarousel onExpand={setSelectedImage} 
            ...
            <div className="flex flex-col md:flex-row">
              ...
              <div className="w-full md:w-3/5 p-6 md:p-12 flex flex-col justify-center">
                ...
                <div className="space-y-6 text-[#f3e8d8] text-lg leading-relaxed mb-10">
                  ...
                  <p>
                    За плечами более 100 успешно выполненных заказов — от восстановления хрупких семейных архивов до создания стильных коммерческих каталогов.
                  </p>
                </div>
                
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 mt-4">
                  <div className="p-6 rounded-2xl bg-[rgba(255,249,240,0.03)] border-glass flex flex-col justify-center">
                    ...
                    <p className="text-sm text-[#f3e8d8] leading-relaxed">
                      Выпускница онлайн-курса «НЕЙРОМАШИНА».<br/>
                      Программа: «Менеджер нейросетей».<br/>
                  ...
                  <div className="aspect-[3/4] rounded-2xl border-glass overflow-hidden relative group bg-[#1c1715]">
                    <img src="/diploma25613405.png" alt="Сертификат" className="w-full h-full object-cover opacity-60 group-hover:opacity-100 transition-opacity duration-500" />
